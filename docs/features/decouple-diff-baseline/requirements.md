...
- **Brand-new docs still clear on a plain read** (plan review): the "stays until reviewed" rule applies to *updates*; a first-sighting doc has no diff to review, so a normal open acknowledges it (no pointless "View changes" click). Mechanically: a plain view advances the marker only when the doc has a single version.
