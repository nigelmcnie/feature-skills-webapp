...
- **Audit.** Comment submit and integrate each write a row to the append-only `events` table — a small, deliberate audit addition. (Synthesis submit only broadcasts over SSE; it does not log an event, so this is a new convention rather than a mirror of synthesis.)
...
### Capture comments in the webapp by reading the iframe's exposed state

Give the webapp a comment-capture surface that parallels the synthesis one already shipped: a submit affordance on the `doc-view` shell, which POSTs to a write endpoint that persists into the `comments` table, and a companion read endpoint, keyed by the document's source path, returning the doc's active comments.

The shell does **not** scrape the rendered DOM for the comments: the spine template keeps comments in a JavaScript array, and the rail only shows truncated, escaped projections, so the real payload isn't reconstructable from the DOM. Nor can the shell read the array as a window property — it's a lexical `const`, invisible on `contentWindow`. So the spine template (`requirements` / `plan`) gains *one* backwards-compatible line exposing the array on the window object (e.g. `window.__fsComments = comments`), and the same-origin, non-sandboxed shell reads that — the full, untruncated payload. This is a minimal, deliberate template change, acceptable because the README already version-pins `feature-skills` and the webapp, so a coordinated bump is an established pattern rather than a new burden. It is the smallest cooperation that lets the shell read what the developer actually entered.
...
The write endpoint (submit comments read from the iframe window's exposed array via the `doc-view` shell, persist to `comments` with replace-active-set semantics, follow the established `503/404/400/200` admin-route contract, length-cap, log an `events` row and broadcast a `changed` event), the `doc-view` comment-submit affordance, the one-line `window.__fsComments` addition to the `feature-skills` spine template (the only cross-repo touch in this phase), and the read endpoint keyed by source path returning a doc's active comments. Testable end-to-end over HTTP and by manual submit. This is `comment-capture` (#8) folded in.
...
- **Comment submit reads the iframe's exposed JS state, not the DOM** (round 1, corrected at plan). The rail DOM holds only truncated, escaped projections, so it can't be scraped; and the round-1 plan to read `iframe.contentWindow.comments` doesn't work either — a lexical `const` isn't a window property. Resolved at plan time (Nigel): the spine template gains one backwards-compatible line exposing the array on the window object (`window.__fsComments`), which the same-origin shell reads. A minimal, deliberate template change, cheap given the existing version-pinning — reversing the round-1 "no template change" intent for a concrete technical reason.
