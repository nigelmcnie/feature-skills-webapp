...
The wait holds for a finite server-side timeout and then returns `submitted=false`, so the agent is never blocked forever and the existing re-call path still works. The timeout is a named constant defaulting to 240 s, chosen to sit just under the 5-minute Anthropic prompt-cache TTL so a client reconnecting after each hold stays cache-warm. Because the hold is event-driven — a real submission wakes the held connection instantly via the broadcaster — a longer hold costs nothing in responsiveness while cutting reconnects roughly 10× versus the original 25 s. There is no reverse proxy in front of the server today; a *future* reverse proxy must set `proxy_read_timeout` ≥ the hold, and the skill side already re-issues the wait on a clean timeout return and degrades to a 5 s short poll if the hold is ever refused, so it fails safe. Cleanup of the registered queue is guaranteed by this bounded timeout via a `try/finally` unregister, *not* by client disconnect detection — whether a plain JSON long-poll handler is cancelled on client disconnect is unverified for this stack, so a dead client's queue is reclaimed at timeout, at worst one window late.
