...
- **Connection longevity.** A long-held HTTP request needs to survive the reverse-proxy/client timeouts of the eventual hosted deployment; the timeout should be chosen with that in mind, and the skill side should re-issue the wait on a clean timeout return.
