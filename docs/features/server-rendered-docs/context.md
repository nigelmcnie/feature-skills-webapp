...
1. **Where does the doc presentation layer live now?** The templates' CSS/JS (TOC, highlighting, comment-rail, sticky footer) has to go somewhere when we stop iframing a self-contained file. Does the webapp own one canonical doc stylesheet + template, and do the per-doc-type section manifests from F1 drive the layout/ordering?
