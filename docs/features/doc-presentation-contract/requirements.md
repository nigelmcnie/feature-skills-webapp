...
- **extra_css precedence (R2):** extra_css uses CSS `@scope (#doc-main)` plus source order — not `@layer`. The model is additive: the canonical base rules still apply per-property and extra_css adjusts on top. To override an existing canonical rule a skill matches its specificity (or simply adds new properties); the contract advertises this model. `@layer` is a noted fallback if specificity collisions ever prove painful. This deliberately deviates from the R1 indicative `@layer` leaning, since the hatch's job is additive tweaks, not wholesale low-specificity overrides.
